...
function tenantBySlug(slug) {
  return db.prepare("SELECT id, slug, name, powered_by, bg_pc, bg_sp, announcement FROM tenants WHERE slug=?").get(slug);
}
...
app.get("/api/:slug/images", requireLoginForSlug, (req, res) => {
  ...
  const rows = db.prepare("SELECT id, filename, start_at, end_at, created_at FROM images WHERE tenant_id=? ORDER BY created_at DESC")
    .all(tenant.id)
...
app.post("/api/:slug/images", requireLoginForSlug, upload.single("image"), (req, res) => {
  ...
  const info = db.prepare("INSERT INTO images (tenant_id, filename, start_at, end_at, created_at) VALUES (?, ?, ?, ?, ?)")
    .run(tenant.id, req.file.filename, start_at, end_at, Date.now());
  res.json({ ok: true, id: info.lastInsertRowid });
});
...
app.post(
  ...
  (req, res) => {
    ...
    if (pc && tenant.bg_pc) {
      const old = path.join(__dirname, "uploads", tenant.bg_pc);
      if (fs.existsSync(old)) fs.unlinkSync(old);
    }
    if (sp && tenant.bg_sp) {
      const old = path.join(__dirname, "uploads", tenant.bg_sp);
      if (fs.existsSync(old)) fs.unlinkSync(old);
    }
...
app.post("/api/:slug/backgrounds/clear", requireLoginForSlug, (req, res) => {
  ...
  if (target === "pc" || target === "all") {
    if (tenant.bg_pc) {
      const p = path.join(__dirname, "uploads", tenant.bg_pc);
      if (fs.existsSync(p)) fs.unlinkSync(p);
    }
    nextPc = null;
  }
  if (target === "sp" || target === "all") {
    if (tenant.bg_sp) {
      const p = path.join(__dirname, "uploads", tenant.bg_sp);
      if (fs.existsSync(p)) fs.unlinkSync(p);
    }
    nextSp = null;
  }
...
app.get("/api/:slug/meta", (req, res) => {
  ...
  const tenant = tenantBySlug(slug);
  if (!tenant) return res.status(404).json({ error: "tenant not found" });
  res.json({ slug: tenant.slug, name: tenant.name, powered_by: tenant.powered_by || "", announcement: tenant.announcement || "", bg_pc_url: tenant.bg_pc ? `/uploads/${tenant.bg_pc}` : "", bg_sp_url: tenant.bg_sp ? `/uploads/${tenant.bg_sp}` : "" });
});
app.get("/api/:slug/active-images", (req, res) => {
  ...
  if (!tenant) return res.status(404).json({ error: "tenant not found" });
  const now = Date.now();
  const rows = db.prepare("SELECT id, filename, start_at, end_at FROM images WHERE tenant_id=? AND start_at <= ? AND end_at >= ? ORDER BY start_at ASC")
    .all(tenant.id, now, now)
    .map(r => ({ id: r.id, url: `/uploads/${r.filename}`, start_at: r.start_at, end_at: r.end_at }));
  res.json({ now, tenant: { slug: tenant.slug, name: tenant.name, powered_by: tenant.powered_by || "", announcement: tenant.announcement || "", bg_pc_url: tenant.bg_pc ? `/uploads/${tenant.bg_pc}` : "", bg_sp_url: tenant.bg_sp ? `/uploads/${tenant.bg_sp}` : "" }, images: rows });
});
